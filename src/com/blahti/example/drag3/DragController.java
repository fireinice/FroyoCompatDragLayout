...
public class DragController {
  public enum DragStatus {
    ON_NORMAL, ON_DRAG, ON_DRAG_PREPARATION
  }

  public DragStatus mDragStatus = DragStatus.ON_NORMAL;
  ...
  public void setDragBackgroundColor(int color) {
    mBackgroundColor = color;
  }
  
  public boolean prepareDrag(View v, DragSource source) {
    boolean doDrag = source.allowDrag ();
    if (!doDrag) return false;
    mDragStatus = DragStatus.ON_NORMAL;
    mOriginator = v;

    mViewSnapshot = getViewBitmap(v);

    if (null == mViewSnapshot) {
      // out of memory?
      return false;
    }
    mDragStatus = DragStatus.ON_DRAG;
    return true;
  }

  private void refreshView(View v) {
    if (v instanceof AbsListView) {
      ((AbsListView)v).invalidateViews();
    } else {
      v.invalidate();
    }
  }

  public void doDrag(View v) {
    switch(mDragStatus) {
    case ON_NORMAL:
      return;
    case ON_DRAG_PREPARATION:
      this.prepareDrag(v, mDragSource);
      this.refreshView(v);
      break;
    case ON_DRAG:
      this.startDrag(v, mDragSource, mDragInfo, mDragAction);
      break;
    }
  }

  public void startDragProcess(View v, DragSource source, Object dragInfo, int dragAction, DragStatus aStatus) {
    mDragStatus = aStatus;
    mDragSource = source;
    mDragInfo = dragInfo;
    mDragAction = dragAction;
    this.refreshView(v);   // this.doDrag(v);
  }
  ...
  public void startDrag(View v, DragSource source, Object dragInfo, int dragAction) {
    ...
    startDrag(mViewSnapshot, screenX, screenY, 0, 0,
	      mViewSnapshot.getWidth(), mViewSnapshot.getHeight(),
	      source, dragInfo, dragAction);
    mViewSnapshot.recycle();
    mViewSnapshot = null;
  ...
  public void startDrag(Bitmap b, int screenX, int screenY,
    ...
    dragView.setBackgroundColor(mBackgroundColor);
